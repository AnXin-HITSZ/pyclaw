...
from openclaw.tools.sandbox_workspace import create_apply_patch_tool, create_list_files_tool
...
class SandboxWorkspaceToolTests(unittest.IsolatedAsyncioTestCase):
    async def test_sandbox_tool_reads_base_url_from_execution_context_metadata(self):
        ...
        with patch("openclaw.tools.sandbox_workspace._sandbox_get", return_value={"items": []}) as sandbox_get:
            outcome = await execute_tool_call(
                ToolCallBlock(id="call_1", name="list_files", input={"path": "."}),
                registry,
                context,
            )
    ...
    async def test_apply_patch_uses_arguments_and_context_metadata(self):
        registry = ToolRegistry()
    ...
    async def test_write_file_uses_json_payload_and_context_metadata(self):
        from openclaw.tools.sandbox_workspace import create_write_file_tool
