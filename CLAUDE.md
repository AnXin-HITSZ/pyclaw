...
- **MySQL：** Service `saas-claw-mysql:3306`，用户 `saas_claw`，数据库 `saas_claw_control` / `saas_claw_runtime`
...
6. InternalServiceAuthFilter 审查（已确认保留：Filter 层集中管控 /api/internal/* 是正确架构，后续扩展时不会漏校验）
7. 前端适配新 API 路径
