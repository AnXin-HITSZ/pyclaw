...
/**
 * Validates internal service-to-service calls routed through the Gateway.
 *
 * All {@code /api/internal/**} requests must carry
 * {@code Authorization: Bearer <internal-token>} where the token matches
 * {@code gateway.runtime.internal-token}.
 *
 * The filter creates an {@code INTERNAL_SERVICE} principal — it is NOT a user
 * JWT and must never be treated as a user login session.
 */
