...
import com.claw.saas.runtime.repository.ToolApprovalRequestRepository;
import com.claw.saas.runtime.service.ToolApprovalService;
...
public class ToolApprovalServiceImpl implements ToolApprovalService {
    ...
    public ToolApprovalServiceImpl(
    ...
    @Override
    public ToolApprovalRequestEntity requireOwnedPending(String clawId, String approvalId, AuthenticatedPrincipal principal) {
        return requireOwnedActionable(clawId, approvalId, principal, null);
    }

    @Override
    public ToolApprovalRequestEntity requireOwnedActionable(String clawId, String approvalId,
    ...
    @Override
    @Transactional
    public ToolApprovalRequestEntity markApprovedForResume(ToolApprovalRequestEntity entity, Authentication authentication,
    ...
    @Override
    @Transactional
    public ToolApprovalRequestEntity markRejectedForResume(ToolApprovalRequestEntity entity, String reason,
    ...
    @Override
    @Transactional
    public ToolApprovalRequestEntity markConsumed(ToolApprovalRequestEntity entity, Authentication authentication) {
    ...
    @Override
    @Transactional
    public ToolApprovalRequestEntity markResumeFailed(ToolApprovalRequestEntity entity, Authentication authentication, String error) {
    ...
    private String serializePreview(Map<String, Object> preview) {
        ...
        try { return objectMapper.writeValueAsString(preview); } catch (JsonProcessingException exc) { return "{}"; }
    }

    private Map<String, Object> deserializePreview(String json) {
        if (json == null || json.isBlank()) return Map.of();
        try { return objectMapper.readValue(json, STRING_OBJECT_MAP); } catch (Exception exc) { return Map.of(); }
    }

    private OffsetDateTime parseExpiresAt(String value, OffsetDateTime fallback) {
        if (value == null || value.isBlank()) return fallback;
        try { return OffsetDateTime.parse(value); } catch (DateTimeParseException exc) { return fallback; }
    }
    ...
    private void writeAudit(Authentication authentication, String action, ToolApprovalRequestEntity entity, boolean success, String error) {
        ...
        auditLogClient.record(actorType, actorId, action, "tool_approval", entity.getId(), success, error);
    }
}
