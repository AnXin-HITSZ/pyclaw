...
import com.claw.saas.runtime.service.SandboxOrchestratorService;
...
public class SandboxOrchestratorServiceImpl implements SandboxOrchestratorService {
    ...
    public SandboxOrchestratorServiceImpl(ObjectProvider<KubernetesClient> clientProvider, SandboxProperties properties) {
    ...
    public void ensureClawSandbox(String userId, String username, String clawId, String clawName) {
        ...
        if (blank(properties.getRunnerImage())) {
            throw new IllegalStateException("runtime.sandbox.runner-image is required when sandbox orchestration is enabled");
        }
        ...
        client().persistentVolumeClaims().inNamespace(namespace).resource(new io.fabric8.kubernetes.api.model.PersistentVolumeClaimBuilder()
                ...
                .withNewResources().addToRequests("storage", new Quantity(properties.getPvcStorageSize())).endResources()
                .endSpec().build()).serverSideApply();

        var resources = new io.fabric8.kubernetes.api.model.ResourceRequirementsBuilder()
                ...
                .addToLimits("memory", new Quantity(properties.getMemoryLimit())).build();

        var container = new io.fabric8.kubernetes.api.model.ContainerBuilder()
                ...
                .withSecurityContext(new io.fabric8.kubernetes.api.model.SecurityContextBuilder()
                        .withAllowPrivilegeEscalation(false)
                        .withReadOnlyRootFilesystem(false)
                        .withNewCapabilities().addToDrop("ALL").endCapabilities().build())
                .build();

        List<LocalObjectReference> imagePullSecrets = blank(properties.getImagePullSecretName())
                ? List.of()
                : List.of(new io.fabric8.kubernetes.api.model.LocalObjectReferenceBuilder().withName(properties.getImagePullSecretName()).build());

        client().apps().deployments().inNamespace(namespace).resource(new io.fabric8.kubernetes.api.model.apps.DeploymentBuilder()
                .withNewMetadata().withName(appName).withLabels(labels).endMetadata()
                ...
                .withSecurityContext(new io.fabric8.kubernetes.api.model.PodSecurityContextBuilder()
                        .withRunAsNonRoot(true).withRunAsUser(10001L).withRunAsGroup(10001L).withFsGroup(10001L)
                ...
                .endSpec().endTemplate().endSpec().build()).serverSideApply();

        client().services().inNamespace(namespace).resource(new io.fabric8.kubernetes.api.model.ServiceBuilder()
                .withNewMetadata().withName(appName).withLabels(labels).endMetadata()
                .withNewSpec().withType("ClusterIP").withSelector(selectorLabels(clawId))
    ...
    public static String clawSecretName(String clawId) { return "claw-secret-" + clawId; }
    public static String userSecretName(String userId) { return "user-secret-" + userId; }
    ...
    private void createOrUpdateSecret(String namespace, String name, Map<String, String> labels, Map<String, String> stringData) {
        ...
        client().secrets().inNamespace(namespace).resource(new io.fabric8.kubernetes.api.model.SecretBuilder()
                .withNewMetadata().withName(name).withLabels(labels).endMetadata()
                .withType("Opaque").withData(encodedData).build()).serverSideApply();
    }
    ...
    public void scaleUserDeployments(String userId, int replicas) {
        ...
        client().apps().deployments().inNamespace(namespace).withLabels(labels).list().getItems()
                .forEach(deployment -> client().apps().deployments().inNamespace(namespace)
    ...
    private void ensureLimitRange(String namespace) {
        ...
        client().limitRanges().inNamespace(namespace).resource(new io.fabric8.kubernetes.api.model.LimitRangeBuilder()
                ...
                .endLimit().endSpec().build()).serverSideApply();
        log.info("ensured limit range: namespace={}", namespace);
    }

    private void ensureNetworkPolicies(String namespace) {
        if (!properties.isNetworkPolicyEnabled()) return;
        client().network().v1().networkPolicies().inNamespace(namespace).resource(new io.fabric8.kubernetes.api.model.networking.v1.NetworkPolicyBuilder()
                .withNewMetadata().withName("deny-ingress-by-default").endMetadata()
                .withNewSpec().withPodSelector(new io.fabric8.kubernetes.api.model.LabelSelectorBuilder().build())
                .withPolicyTypes("Ingress").endSpec().build()).serverSideApply();

        client().network().v1().networkPolicies().inNamespace(namespace).resource(new io.fabric8.kubernetes.api.model.networking.v1.NetworkPolicyBuilder()
                .withNewMetadata().withName("allow-control-plane-to-runner").endMetadata()
                .withNewSpec()
                .withPodSelector(new io.fabric8.kubernetes.api.model.LabelSelectorBuilder()
                        .addToMatchLabels(COMPONENT_LABEL, "sandbox-runner").build())
                ...
                .withIngress(new io.fabric8.kubernetes.api.model.networking.v1.NetworkPolicyIngressRuleBuilder()
                        ...
                        .withPorts(new io.fabric8.kubernetes.api.model.networking.v1.NetworkPolicyPortBuilder()
                                .withProtocol("TCP")
                                .withPort(new IntOrString(properties.getRunnerPort())).build())
                        .build())
                .endSpec().build()).serverSideApply();
    ...
    private void ensureSandboxServiceAccount(String namespace) {
        client().serviceAccounts().inNamespace(namespace).resource(new io.fabric8.kubernetes.api.model.ServiceAccountBuilder()
                .withNewMetadata().withName("sandbox-runner").endMetadata()
                .withAutomountServiceAccountToken(false).build()).serverSideApply();
    ...
    private void ensureImagePullSecret(String targetNamespace) {
        ...
        client().secrets().inNamespace(targetNamespace).resource(new io.fabric8.kubernetes.api.model.SecretBuilder()
                ...
                .withType(source.getType()).withData(source.getData()).build()).serverSideApply();
    ...
    private String imagePullSecretSourceNamespace() {
        ...
        if (blank(sourceNamespace)) sourceNamespace = System.getenv("POD_NAMESPACE");
        if (blank(sourceNamespace)) {
            throw new IllegalStateException("runtime.sandbox.image-pull-secret-source-namespace is required when sandbox image pull secret is configured");
        }
    ...
    private KubernetesClient client() {
        return clientProvider.getIfAvailable(() -> {
            throw new IllegalStateException("Kubernetes client is unavailable; enable runtime.sandbox.enabled only in a Kubernetes runtime");
        });
    }
    ...
    private String resourceName(String prefix, String id) { return dnsName(prefix + "-" + id); }
    private String dnsLabel(String value) { return dnsName(value == null ? "unknown" : value); }
    ...
    private String dnsName(String value) {
        ...
        if (normalized.isBlank()) normalized = "x";
        if (normalized.length() > 63) normalized = normalized.substring(0, 63).replaceAll("-$", "");
        return normalized;
    }
